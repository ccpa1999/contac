...
class modeloCarteras extends conexion
{
    ...
    private function paginaInicio($datos)
    {
        ...
        {
            $resultado['historial'] = 
                $this->obtenerInformacionHistoricoGestion(
                    $resultado['cliente']['cliente'][0]['cedula'], 
                    $datos['cartera']);
        }
    ...
    private function buscarDeudor($datos)
    {
        ...
        if (isset($datos['key']) && ($_SESSION['carteraActual'] == 4 || 
            $_SESSION['carteraActual'] == 5 || $_SESSION['carteraActual'] == 13)) 
        {
            $resultado['cliente'] = $this->obtenerInformacionClientesMultiplay($datos['cartera'], 
                $datos['datoBusqueda'], $datos['tipo']);

            $resultado['historial'] = $this->obtenerInformacionHistoricoGestionMultiplay(
                $resultado['cliente']['cliente'][0]['cedula'], $datos['cartera']);
        }
        else
        {
            $resultado['cliente'] = $this->obtenerInformacionClientesParametro($datos['cartera'], 
                $datos['datoBusqueda'], $datos['tipo']);

            if (isset($resultado['cliente']['cliente'][0]['cedula'])) 
            {
                $resultado['historial'] = $this->obtenerInformacionHistoricoGestion(
                    $resultado['cliente']['cliente'][0]['cedula'], $datos['cartera']);
            }
        }
    ...
    private function obtenerInformacionClientes($cartera)
    {
        ...
        $query = "SELECT d.cedula FROM deudores AS d, obligaciones AS o 
        WHERE d.cedula = o.cedula_deudor AND o.cartera = $cartera 
            AND d.estado = 1 AND d.cedula NOT IN ".
            "(SELECT bg.cedula_deudor FROM bloqueo_gestion AS bg
            WHERE bg.cedula_deudor = o.cedula_deudor 
             AND bg.id_bloqueo = CONCAT('1-', '$cartera')) LIMIT 1;";
    ...
    private function obtenerInformacionClientesParametro($cartera, $parametro, $tipo)
    {
        ...
        {
            case 'cedula':
                $query = "SELECT cedula as cedula_deudor FROM deudores 
                    WHERE cedula = '$parametro'";
                break;
            case 'numero_obligacion':
                $query = "SELECT cedula_deudor FROM obligaciones 
                    WHERE numero_obligacion = '$parametro'";
                break;
            case 'telefono':
                $query = "SELECT cedula_deudor FROM telefonos 
                    WHERE telefono = '$parametro'";
                break;
        }
    ...
    private function obtenerInformacionClientesMultiplay($cartera, $parametro, $tipo)
    {
        ...
        if ($cedula != '') 
        {
            ...
            foreach ($return['obligaciones'] as $key => $obligaciones) 
            {
                $query = "SELECT * FROM pagos WHERE obligacion = '" . $obligaciones['numero_obligacion'] 
                    ."' AND cliente_pago = '$cartera'";
    ...
    private function obtenerInformacionHistoricoGestion($cedula, $cartera)
    {
        ...
        FROM homologado_accion a, homologado_efecto e, homologado_contacto c, historico_gestion h 
        WHERE h.cedula_deudor = '$cedula' AND h.cliente_id = '$cartera' AND h.accion = a.id 
            AND h.efecto = e.id AND h.contacto = c.id ORDER by fecha_gestion DESC";
    ...
    private function obtenerEfectosContacto($datos)
    {
        ...
        $query = "SELECT m.motivo, m.id FROM motivos_no_pago m, arbol_motivos_no_pago a "
            . "WHERE a.id_contacto = '" . $datos['contacto'] . "' AND a.id_motivo_no_pago = m.id
                AND a.id_cliente = '" . $datos['cartera'] . "' AND a.estado = 1 ORDER BY m.motivo";
    ...
    private function cargaAsignacionUpdate($datos)
    {
        ...
        /**
         * La lectura de una línea se cortará si esa línea en el .csv excede el límite
         * de caracteres que establece el segundo parámetro, por lo tanto, 
         * el array que contiene el registro de esa línea quedará incompleto. Dejando 
         * la parte que excede la cantidad de caracteres en un array separado.
         * 
         * Para evitar este problema, sólo hay que aumentar el número límite de caracteres 
         * por línea.
         * 
         */
        while (($datos = fgetcsv($handle, 2000, ";")) !== FALSE) 
        {
            $query .= utf8_encode("\n\nINSERT INTO tmp (campo1, campo2, campo3, campo4, campo5, campo6, campo7,
        ...
        $query .= "\n\nINSERT INTO deudores (nombre, cedula, estado) " .
            "SELECT campo7, campo1, '1' FROM tmp ON DUPLICATE KEY
                  UPDATE estado = '1';";

        $query .= "\n\nUPDATE obligaciones SET estado = '0' WHERE cartera = '$cartera' "
            . "AND (fecha_vigencia_asignacion = '' "
            . "OR fecha_vigencia_asignacion = CURDATE());";

        $query .= "\n\nINSERT INTO obligaciones (cedula_deudor, cartera, producto, numero_obligacion, entidad, "
        ...
        $query .= "\n\nINSERT INTO direcciones (cedula_deudor, tipo_direccion, ciudad, direccion, estado) "
        ...
        for ($i = 62; $i <= 67; $i++) 
        {
            ...
            $query .= "\n\nINSERT INTO telefonos (cedula_deudor, tipo_telefono, telefono, estado) "
                . "SELECT campo1, '" . utf8_decode($tipo) . "', campo" . $i . ", '1' FROM tmp
                        WHERE campo" . $i . " != '' ON DUPLICATE KEY "
                . "UPDATE estado = '1';";
        }

        $query .= "\n\nINSERT INTO correos (cedula_deudor, tipo_correo, correo, estado) "
    ...
    {
        $datos = $this->obtenerInformacionHistoricoGestion($datos['cedula_deudor'], 
            $datos['cartera']);
    ...
    private function administracionArbol($datos)
    {
        ...
        $query = "SELECT id, homologado FROM homologado_accion WHERE id_cliente = '" . $datos['cartera'] . "' AND estado = '1' ORDER BY homologado";
        ...
        switch ($datos['tipo']) 
        {
            case 'accion':
                ...
                $query = "SELECT h.id as id, h.homologado FROM homologado_contacto h, arbol_contacto a WHERE a.id_cliente = '" . $datos['cartera'] . "'"
                    . "AND a.id_contacto = h.id AND a.id_accion = '" . $datos['parametro'] . "' AND a.estado = '1' AND h.estado = '1'  ORDER BY h.homologado";
            ...
            case 'contacto':
                ...
                $query = "SELECT h.id, h.homologado FROM homologado_efecto h, arbol_efecto a WHERE a.id_cliente = '" . $datos['cartera'] . "'"
                    . "AND a.id_efecto = h.id AND a.id_contacto = '" . $datos['parametro'] . "' AND a.estado = '1' AND h.estado = '1'  ORDER BY h.homologado";
                $resultado['asignadas'] = $this->row($query);
                break;
            case 'motivo':
                $query = "SELECT id, motivo FROM motivos_no_pago WHERE id_cliente = '" . $datos['cartera'] . "' AND estado = '1'  ORDER BY motivo";
                ...
                $query = "SELECT m.id, m.motivo FROM motivos_no_pago m, arbol_motivos_no_pago a WHERE a.id_cliente = '" . $datos['cartera'] . "'"
                    . "AND a.id_motivo_no_pago = m.id AND a.id_contacto = '".$datos['parametro']."' AND a.estado = '1'  ORDER BY m.motivo";
                $resultado['asignadas'] = $this->row($query);
                break;
        }
    ...
    {        
        ...
        {
            case 'accion':
                $query = "UPDATE arbol_contacto SET estado = 0 WHERE id_accion = '".$datos['contacto']
                    ."' AND id_cliente = '" . $datos['cartera'] . "'";
                $resultado = $this->ejecutar2($query);
                ...
                break;
            case 'contacto':
                $query = "UPDATE arbol_efecto SET estado = 0 WHERE id_contacto = '".$datos['contacto'] 
                    ."' AND id_cliente = '".$datos['cartera']."'";
                $resultado = $this->ejecutar2($query);
                ...
                break;
            case 'motivo':
                $query = "UPDATE arbol_motivos_no_pago SET estado = 0 WHERE id_contacto = '".$datos['contacto'] 
                    ."' AND id_cliente = '" . $datos['cartera'] . "'";
                $resultado = $this->ejecutar2($query);
    ...
    {        
        $query = "SELECT documento_titular, fecha  FROM agendamiento WHERE usuario = '" . $_SESSION['usuario'] 
            ."' AND fecha BETWEEN DATE_SUB(NOW(), INTERVAL 1 HOUR) AND DATE_ADD(NOW(), INTERVAL 10 MINUTE)". 
    ...
    private function informeProductividad($datos)
    {
        ...
        $resultado['clientes'] = $this->row($query);
        ...
        $resultado['promesas'] = $this->row($query);
        ...
        $resultado['posibles'] = $this->row($query);
        ...
        $resultado['directos'] = $this->row($query);

        $query = "SELECT gestor, COUNT(efecto) as gestiones FROM `historico_gestion` WHERE cliente_id = '" . $datos['cartera'] . "' AND fecha_gestion BETWEEN '" . $datos['fecha_inicial'] . " 00:00:00'
                AND '" . $datos['fecha_final'] . " 23:59:59' GROUP by gestor;";
        $resultado['gestiones'] = $this->row($query);
        ...
        $fp = fopen('../../public/archivos/descargas/'.$datos['cartera']
            .'/informe_' . $datos['informe'].' '.date("Y-m-d").'.csv', 'w');
        ...
        for ($i = 0; $i < count($resultado['gestiones']); $i++)
        {
            $array['agente'] = $resultado['gestiones'][$i]['gestor'];
            
            $clave_gestionado = $this->buscarEnArray($array['agente'], $resultado['clientes'], 'gestor');
            $array['gestionados'] = $resultado['clientes'][$clave_gestionado]['cliente'];

            $clave_compromiso = $this->buscarEnArray($array['agente'], $resultado['promesas'], 'gestor');
            $array['compromisos'] = $this->valorProductividad($resultado['promesas'], 
                $clave_compromiso, 'promesa');

            $clave_posible = $this->buscarEnArray($array['agente'], $resultado['posibles'], 'gestor');
            $array['posibles'] = $this->valorProductividad($resultado['posibles'], $clave_posible, 'posible');

            $clave_directo = $this->buscarEnArray($array['agente'], $resultado['directos'], 'gestor');
            $array['directos'] = $this->valorProductividad($resultado['directos'], $clave_directo, 'contacto');

            $clave_gestiones = $this->buscarEnArray($array['agente'], $resultado['gestiones'], 'gestor');
            $array['gestiones'] = $this->valorProductividad($resultado['gestiones'], $clave_gestiones, 'gestiones');
    ...
    private function buscarEnArray($buscar, $array, $columna)
    {
        return array_search($buscar, array_column($array, $columna));
    }

    private function valorProductividad($array, $indice, $columna)
    {
        if ($indice == '' && $indice !== 0)
            return '0';
        else
            return $array[$indice][$columna];
    }
    ...
    private function informeMejorGestion($datos)
    {
        ...
        $fp = fopen('../../public/archivos/descargas/'.$datos['cartera'].'/informe_'
            .$datos['informe'].' '.date("Y-m-d").'.csv', 'w');
    ...
    private function administracionCartera($datos)
    {
        ...
        $query = "SELECT id, accion FROM accion WHERE estado = '1' ORDER BY accion ASC";
        $resultado = $this->row($query);
        $return['accion'] = $resultado;

        $query = "SELECT id, contacto FROM contacto WHERE estado = '1'  ORDER BY contacto ASC";
        $resultado = $this->row($query);
        $return['contacto'] = $resultado;

        $query = "SELECT id, efecto FROM efecto WHERE estado = '1' ORDER BY efecto ASC";
        $resultado = $this->row($query);
    ...
    private function guardarTiempoMuerto($datos)
    {
        $query = "INSERT INTO pausas (tipo_pausa, tiempo_pausa, usuario, fecha_pausa, cartera) "
            . "VALUES ('" . $datos['tipo'] . "', '" . $datos['tiempo'] . "', '" . $_SESSION['usuario'] 
            ."', NOW(), '" . $datos['cartera'] . "')";
    ...
    private function informeDemografico($datos)
    {
        ...
        $query = "SELECT cedula_deudor FROM historico_gestion"
            ." WHERE fecha_gestion BETWEEN '" . $datos['fecha_inicial'] . " 00:00:00' AND '" . $datos['fecha_final'] . " 23:59:59'"
    ...
    {
        $query = "SELECT * FROM guiones_gestion WHERE id_efecto = '" . $datos['dato'] . 
            "' AND id_cliente = '" . $datos['cartera'] . "'";
