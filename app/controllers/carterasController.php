...
class carterasController {
    ...
    public function __construct($datos)
    {
        ...
        if (isset($_SESSION['usuario'])) 
        {
            if (isset($datos['cartera']))
                $this->validarCartera($datos['cartera']);
    ...
    public function paginaInicio($datos)
    {
        ...
        $plantilla = $this->plantillas->render('carteras/moduloGestion.html', 
            Array('cartera' => $data['cartera'],
                'cliente' => $data['cliente'],
                'gestion' => $data['gestion'],
                'historial' => (isset($data['historial'])) ? $data['historial'] : '',
    ...
    {
        if(isset($datos['key']))
          $datos['cartera'] = $_SESSION['carteraActual'];
        ...
        else 
        {
            ...
            <script>
                $.confirm({
                    ...
                    theme: 'dark',
                    animation: 'scaleX',
                    content: 'El dato ingresado no coincide con ningún titular para esta campaña',
                    buttons: {
                        Ok: function () {
                            window.history.back();
                        }
    ...
    /**
     * Función que construye el módulo de gestión de acuerdo al criterio de busqueda filtrando por la edad de mora que se le ha asignado al usuario
     *
     * @param Array $datos contiene el criterio y el tipo de dato para la busqueda
     * @author Jonnathan Murcia <jjmurciab@gmail.com>
     * @return String plantilla renderizada con la información requerida
     */
    public function buscarDeudorEdadMora($datos)
    {
         $data = $this->claseCarteras->controlador($datos);
         $resultado = (!empty($data['cliente']['obligaciones'])) ? 'ok' : 'fallo';
         if ($resultado == 'ok') {
    ...
    public function exportarDeudorDemografico($datos){
        ...
        $plantilla = $this->plantillas->render('carteras/formulariosVarios.html', ['datos' => $resultado, 'parametro' => $datos['metodo']]);
        echo $plantilla;
    }
     ...
     * @return String plantilla renderizada con la información requerida
     */
    public function formulariosSimuladores($datos)
    {
        $resultado = $this->claseCarteras->controlador($datos);
        $plantilla = $this->plantillas->render('carteras/simuladores.html', ['parametro' => $datos['parametro'], 'carteraActual' => $datos['cartera'], 'datos' => $resultado]);
        echo $plantilla;
    }
     ...
     */
    public function formularioReestructuracion($datos)
    {
        $resultado = $this->claseCarteras->controlador($datos);
        $plantilla = $this->plantillas->render('carteras/formulariosReestructuracion.html', ['datos' => $datos, 'parametro' => $datos['parametro'], 'resultado' => $resultado ]);
        echo $plantilla;
    }
     ...
     *                fallo: La consulta retorna un ingreso incorrecto
     */
    public function guardarFormularioReestructuracion($datos)
    {
        $resultado = $this->claseCarteras->controlador($datos);
    ...
    public function iniciarPausa($datos)
    {
        ...
        $plantilla = $this->plantillas->render('carteras/formulariosVarios.html', 
        ['parametro' => $datos['parametro'], 'carteraActual' => $datos['cartera'],
            'datos' => $datos]);
    ...
    public function administracionArbol($datos)
    {
        ...
        $plantilla = $this->plantillas->render('carteras/administracionArbol.html',
             ['arbol' => $arbol]);
    ...
    public function administracionInformes($datos)
    {
        ...
        $plantilla = $this->plantillas->render('administracionInformes.html',
    ...
    {
        $plantilla = $this->plantillas->render('carteras/formulariosCreacion.html', 
            ['parametro' => $datos['parametro'],
            'datos' => $datos]);
    ...
    public function formulariosCreacionDemografico($datos)
    {
        ...
        $plantilla = $this->plantillas->render('carteras/formulariosVarios.html', 
            ['datos' => $datos, 'parametro' => $datos['parametro']]);
    ...
    public function creacionDemografico($datos) {
        ...
        $plantilla = $this->plantillas->render('carteras/resultadoFormularios.html', 
            ['parametro' => $datos['accion'], 'datos' => $respuesta['datos']]);
    ...
    public function cargarTarea($datos)
    {
        ...
        $return = $this->claseCarteras->controlador(array('ruta' => $ruta, 
            'metodo' => 'cargarTarea', 'cartera' => $datos['cartera'],
            'datos' => $datos));
    ...
    public function administracionCartera($datos)
    {
        ...
        $plantilla = $this->plantillas->render('carteras/administracionCartera.html', 
            ['cartera' => $datos['carteraActual'], 'datos' => $data]);
    ...
    public function seleccionarObligacionesGestion($datos)
    {
        ...
        $plantilla = $this->plantillas->render('carteras/formulariosVarios.html', 
            ['parametro' => 'seleccionarObligacionesGestion',
            'carteraActual' => $datos['cartera'], 
    ...
    /**
     * Metodo para realiza la busqueda de la referencia de pago y los pagos realizadas a la obligación
     *
     * @param array $datos contiene la información para el cargue de la cartera
     * @author Jonnathan Murcia <jjmurciab@gmail.com>
     * @return string $plantilla imprime la plantilla con los datos cargados
    ...
    public function busquedaReferencia($datos) {
        ...
        $plantilla = $this->plantillas->render('carteras/resultadoFormularios.html', ['parametro' => 'busquedaReferencia',
    ...
    public function administrarObligatoriedad($datos) {
        ...
        $resultado = $this->claseCarteras->controlador($datos);
        $plantilla = $this->plantillas->render('carteras/resultadoFormularios.html', ['parametros' => $resultado,
    ...
    public function guardarGuion($datos) {
        $resultado = $this->claseCarteras->controlador($datos);
    ...
    public function generarPDFReestructuracion($datos) {
        $resultado = $this->claseCarteras->controlador($datos);
        $plantilla = $this->plantillas->render('pdf/formularioReestructuracionDomPDF.html', ['resultado' => $resultado]);
        $pdfDom = new GenerarPDFdom();
        $ruta = $pdfDom->formatoSolicitudReestructuracion(array('plantilla' => $plantilla));
        echo $ruta;
    }

    public function panelSoporte($datos)
    {
        $errores = $this->claseCarteras->controlador($datos);
        $plantilla = $this->plantillas->render('carteras/formulariosVarios.html', ['parametro' => 'panelSoporte', 'errores' => $errores]);
        echo $plantilla;
    }

    /*public function submitSoporte($datos)
    {
        $mail = new enviarEmail();
        $resultado = $mail->enviarSoporte($datos);
        echo json_encode($resultado);
    }*/

    public function panelAsignacionMora($datos){
        $resultado = $this->claseCarteras->controlador($datos);
        $plantilla = $this->plantillas->render('carteras/formulariosVarios.html', ['parametro' => 'panelAsignacionMora',
        'resultado' => $resultado]);
        echo $plantilla;
    }
    ...
    public function buscarAsignacionMora($datos){
        $resultado = $this->claseCarteras->controlador($datos);
        $plantilla = $this->plantillas->render('resultadoBusquedas.html', ['parametro' => 'buscarAsignacionMora',
        'resultado' => $resultado]);
        echo $plantilla;
    }
    ...
    public function obtenerChats($datos){
        $mensajes = $this->claseCarteras->controlador($datos);
        $plantilla = $this->plantillas->render('resultadoBusquedas.html', ['parametro' => $datos['parametro'], 'resultado' => $mensajes ]);
        echo $plantilla;
    }

    public function notificacionChats($datos){
        $alerta = $this->claseCarteras->controlador($datos);
        echo $alerta[0]['alerta'];
    }

    public function buscarChats($datos)
    {
        $resultados = $this->claseCarteras->controlador($datos);
        $plantilla = $this->plantillas->render('resultadoBusquedas.html', [
            'resultado' => $resultados,
            'parametro' => 'busquedaChats'
        ]);
        echo $plantilla;
    }

    public function panelChat($datos){
        $panel = $this->claseCarteras->controlador($datos);
        $mensajes = $this->claseCarteras->controlador(['metodo' => 'obtenerChats' ,'receptor' => $_SESSION['id_usuario'] . ',' .$_SESSION['usuario']]);
        $plantilla = $this->plantillas->render('carteras/formulariosVarios.html', ['parametro' => $datos['parametro'], 'resultado' => $panel, 'mensajes' => $mensajes]);
        echo $plantilla;
    }
    ...
    public function submitMensaje($datos){
        ...
        $datos['archivo'] = (!empty($_FILES['archivo']['name'])) ?  $_FILES['archivo']['name'] : '';
        $return = $this->claseCarteras->controlador($datos);
        $ruta = '../../public/archivos/cargas/archivosChat/' . $_FILES['archivo']['name'] ;
        if (!file_exists($ruta)) {
            $resultado = move_uploaded_file($_FILES['archivo']['tmp_name'], $ruta);
        }
    }
    ...
    private function validarCartera($cartera)
    {
        try
        {
            ...
            {
                if ($_SESSION['acceso'][$indice]['id_cliente'] == $cartera)
                    break;

                if ($indice == count($_SESSION['acceso']) - 1)
                    throw new Exception('SIN PERMISOS EN LA CARTERA');
            }
        }
        catch(Exception $ex)
        {
            header('Location: ./administracionController.php');
        }
    }
}
