            ...
            if (datos.resultado == 'ok') 
            {
                ...
                $('.tableDemografico').DataTable({
                    ...
                    "language": {
                        ...
                        "zeroRecords": "No hay demográfico",
...
function visualizarTelefonos(telefonoSeleccionado)
{
    ...
    $('#telefonosGestion').val(telefono);

    window.getSelection(telefono);
    document.execCommand("copy");
    
    $('#visualizacionTelefonos').html('<h4><span class="label label-default">'+ 
        '<strong><i class="fa fa-phone" style="color:#5cb85c;"></i> ' +
         telefono + '</strong></span></h4>');
...
function crearParametroArbol() {
    ...
    .done(function (data) {
        mensaje('dark', '¡FELICIDADES!', 'green', 
            'Se guardo el dato correctamente');
    });
}
...
function generarInforme() {
    if ($("#formularioGenerar").parsley().validate() == true) 
    {
        ...
        .done(function (resultado) {
            ...
            mensaje('dark', '¡FELICIDADES!', 'green',
                'Se generó el informe correctamente');
...
function administracionGuion() {
    $.ajax({
        ...
        if (data == 'ok') {
            mensaje('DARK', '¡Atencion!', 'green', 
                'Se he registrado el guión de forma exitosa');
        } else {
            mensaje('DARK', '¡Atencion!', 'red', 
                '¡Ocurrio un error en la creación del guion!');
        }
    });
}
...
function scriptIncial() {
    ...
    $('.tablaHistoricoGestion').DataTable({
        ...
        "language": {
            "lengthMenu": "Mostrar _MENU_",
            "zeroRecords": "<h4 class='text-primary'><b>No ha registro disponible</b><h4>",
            "info": "Mostrando pagina _PAGE_ de _PAGES_",
            "infoEmpty": "No hay registro disponible",
            "search": "Buscar:",
            "paginate": {
...
function buscarDeudorAgendamiento() {
    ...
    url = "../../app/controllers/carterasController.php?tipo=cedula&datoBusqueda=" + 
        thiz.data('cedula') + "&metodo=buscarDeudor&cartera=" + cartera;
...
function resultadoContacto() {
    ...
    .done(function (data) {
        ...
        $.each(data.actividades, function (i, item) {
            $('#actividad_gestion').append(' <option value="' + item.id + '">' + 
                item.actividad + '</option>');
        });
...
function refrescarHistorico() {
    $.ajax({
        ...
        $('.tablaHistoricoGestion').DataTable({
            ...
            "language": {
                "lengthMenu": "Mostrar _MENU_",
                "zeroRecords": "<h4 class='text-danger'><b>No hay registro disponible</b><h4>",
                "info": "Mostrando pagina _PAGE_ de _PAGES_",
                "infoEmpty": "No hay registro disponible",
                "search": "Buscar:",
                "paginate": {
...
$(document).ready(function () {
    ...
    // Tabla de gestiones en moduloGestion.html.php
    $('.tablaHistoricoGestion').DataTable({
        "responsive": true,
        "scrollX": true,
        "order": [[0, "desc"]],
        ...
        "language": {
            "lengthMenu": "Mostrar _MENU_",
            "zeroRecords": "<h4 class='text-danger'><b>No hay registros disponibles</b><h4>",
            "info": "Mostrando pagina _PAGE_ de _PAGES_",
            "infoEmpty": "No hay registro disponible",
            "search": "Buscar:",
            "paginate": {
    ...
    $('.tableDemograficoe').on('dblclick.dt', '.telefonoGestion', function () {
        visualizarTelefonos($(this));
    });
